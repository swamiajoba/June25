...
public class Demo {
	...
	public static void main(String[] args) throws ParseException {
		...
		// select reocrd  = select * from Employee where eid=101;
		Employee upe=session.get(com.model.Employee.class, 101);
		System.out.println("Before update employee is "+upe);
		
		upe.setSalary(50000.00);
		session.saveOrUpdate(upe);
		
		  upe= session.get(com.model.Employee.class,new Integer(101));
		...
		*/
		...
		// code for delete
		Transaction tx=session.beginTransaction();
	    Employee myemp1= session.get(com.model.Employee.class,new Integer(101));
